...
@section('content')
    <div class="min-h-screen bg-gray-50 text-gray-900 antialiased">
            ...
            <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-end">
                <a href="{{ route('dashboard') }}"
                   class="flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-semibold transition-colors text-gray-500 hover:text-gray-900">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 17l-5-5m0 0l5-5m5 5h12" /></svg>
                    Volver al Dashboard
                </a>
            </div>
        </header>
        ...
        <main class="max-w-4xl mx-auto px-6 py-12 md:py-16 animate-fade-in-up">
            ...
            @php
                $usuario = auth()->user();
                $nameParts = explode(' ', $usuario->nombre ?? 'Usuario');
                $initials = strtoupper(substr($nameParts[0], 0, 1) . substr($nameParts[1] ?? '', 0, 1));
            @endphp

            {{-- ⭐️ BLOQUE AVATAR MEJORADO ⭐️ --}}
            <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-200 mb-10 flex flex-col md:flex-row items-center gap-8">
                ...
                <div class="w-24 h-24 rounded-full shadow-lg border-4 border-white ring-4 ring-blue-500/50 flex items-center justify-center overflow-hidden bg-gray-200 flex-shrink-0">
                    @if ($usuario->avatar_url)
                        <img src="{{ $usuario->avatar_url }}" alt="Foto de perfil de {{ $usuario->nombre ?? 'Usuario' }}" class="object-cover w-full h-full">
                    @else
                        <span class="text-3xl font-extrabold text-blue-700">{{ $initials }}</span>
                    @endif
                </div>

                {{-- 2. Formulario de subir imagen --}}
                <div class="flex-grow">
                    <h3 class="text-xl font-bold text-gray-900 mb-4">Foto de Perfil</h3>

                    <form method="POST" action="{{ route('users.upload_avatar') }}" enctype="multipart/form-data"
                          class="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                        @csrf

                        <div class="flex-grow w-full">
                            {{-- Se añade un poco más de contexto al label/ayuda --}}
                            ...
                            <input type="file" id="avatar" name="avatar" accept=".jpg,.jpeg,.png" required
                                   class="block w-full text-sm text-gray-900 border border-gray-300 rounded-xl bg-gray-50 shadow-inner-sm cursor-pointer
                                   file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold
                                   file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition duration-300">
                        ...
                        <button type="submit"
                                class="h-10 text-base font-bold bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-6 shadow-md shadow-blue-500/30 flex-shrink-0 transition duration-300">
                            Subir Foto
                        </button>
                    </form>
                    ...
                    {{-- 3. Botón eliminar --}}
                    @if ($usuario->avatar_url)
                        <form method="POST" action="{{ route('users.delete_avatar') }}" class="mt-3">
                            @csrf
                            @method('DELETE')
            ...
            {{-- ⭐️ FIN: BLOQUE AVATAR MEJORADO ⭐️ --}}
            ...
            <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-200 mb-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                    <div>
                        ...
                        <p class="text-lg font-extrabold text-gray-900">{{ $usuario->nombre ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-bold uppercase tracking-widest text-gray-500 mb-1">Correo Electrónico</p>
                        <p class="text-lg font-semibold text-gray-900">{{ $usuario->correo ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-200 mb-10">
                ...
                <form method="POST" action="{{ route('users.update_profile') }}">
                    ...
                    <div class="flex justify-end pt-4 border-t border-gray-200/70 mt-8">
                        <button type="submit" class="h-12 text-lg font-extrabold bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-8 shadow-lg shadow-blue-500/40 transform hover:scale-[1.005] focus:outline-none focus:ring-4 focus:ring-blue-600/50">
                            Guardar cambios
                        </button>
                    </div>
                </form>
            ...
            <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-200 mb-10">
                ...
                <form method="POST" action="{{ route('users.update_password') }}">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="password" name="current_password" required class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-xl block w-full p-3 shadow-inner-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-300/50 transition duration-300 outline-none">
                    </div>

                    <div class="mb-6">
                        <label class="block mb-2 text-sm font-medium text-gray-500">Nueva contraseña</label>
                        <input type="password" name="password" required class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-xl block w-full p-3 shadow-inner-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-300/50 transition duration-300 outline-none">
                    </div>

                    <div class="mb-6">
                        <label class="block mb-2 text-sm font-medium text-gray-500">Confirmar nueva contraseña</label>
                        <input type="password" name="password_confirmation" required class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-xl block w-full p-3 shadow-inner-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-300/50 transition duration-300 outline-none">
                    </div>

                    <div class="flex justify-end pt-4 border-t border-gray-200/70 mt-8">
                        <button type="submit" class="h-12 text-lg font-extrabold bg-red-600 hover:bg-red-700 text-white rounded-xl px-8 shadow-lg shadow-red-500/40 transform hover:scale-[1.005] focus:outline-none focus:ring-4 focus:ring-red-500/50">
            ...
            <div class="bg-white shadow-xl rounded-2xl p-8 border border-red-400/50">
                ...
                <form method="POST" action="{{ route('users.destroy_account') }}"
                    ...
                    <button type="submit"
                            class="h-12 text-lg font-extrabold bg-red-800 hover:bg-red-900 text-white rounded-xl px-8 shadow-lg shadow-red-800/40">
